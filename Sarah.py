...
from concurrent.futures import ThreadPoolExecutor as sarah_id
...
╠╩╗╠╦╝║ ║ ║ ║╣   ║║ ╦
╚═╝╩╚═╚═╝ ╩ ╚═╝  ╩╚═╝
( Di buat oleh [bold green]SARAH_ID [bold blue]) ''','color(8)')
...
[bold white][[bold green]0[bold white]] Keluar...''','color(8)')
    ...
    if cuih in ['1','01']:
        ...
        if akun in ['1','01']:
           ...
           okeh = open('OK/{}'.format(sarah),'r').read()
           print(okeh)
           exit(0)
        else:
           ...
           for xnxx in list:
               ...
               print('\r  %s%s. %s%s'%(H, iyah, N, xnxx))
           nomor = input(f'\n  {H}• {N}masukan nomor : {H}')
           try:sarah = cantik[int(nomor)-1]
           except Exception as e:exit(e)
           okeh = open('CP/{}'.format(sarah),'r').read()
           print(okeh)
           exit(0)
    elif cuih in ['0','00']:
...
class dump:
    ...
    def followers(self, userid, cookies, sasahdev):
        with requests.Session() as kontol:
               try:
                     self.url = kontol.get("https://i.instagram.com/api/v1/friendships/{}/followers/?count=100&max_id={}".format(userid,sarahdev), headers = {"user-agent":ua_ig}, cookies = {"cookie":cookies})
    ...
    def following(self, userid, cookies, khamdihidev):
          with requests.Session() as kontol:
               try:
                     self.url = kontol.get("https://i.instagram.com/api/v1/friendships/{}/following/?count=100&max_id={}".format(userid, sarahdev), headers = {"user-agent":ua_ig}, cookies = {"cookie":cookies})
                     for self.txt in json.loads(self.url.text)["users"]:
                         if self.txt["username"] in self.id:
                              continue
                         self.id.append(self.txt["username"]+"<=>"+self.txt["full_name"])
